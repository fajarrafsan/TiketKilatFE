...
function FlightsManagement() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        page: String(page),
        size: '10',
        urutanBerdasarkan: 'waktuKeberangkatan',
        arah: direction,
      });
      setPageData(
        await apiGet<SpringPage<Flight>>(
          `/admin/Mengambil-semua-data-penerbangan?${params}`,
        ),
      );
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : 'Data penerbangan belum dapat dimuat.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function saveFlight(event: React.FormEvent<HTMLFormElement>) {
    ...
    setFeedback('');
    if (
      form.kotaKeberangkatan.trim().toLowerCase() ===
      form.kotaTujuan.trim().toLowerCase()
    ) {
    ...
    try {
      if (editing) {
        ...
        await apiPut(
          `/admin/update-data-penerbangan/${editing.id}`,
          updatePayload,
        );
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : 'Penerbangan belum dapat disimpan.',
      );
    } finally {
      setSaving(false);
    }
  ...
  async function deleteFlight(id: number) {
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : 'Penerbangan belum dapat dihapus.',
      );
    } finally {
      setDeleting(null);
    }
  ...
  async function showPassengers(flight: Flight) {
    ...
    setPassengerTitle(
      `${flight.maskapai} · ${flight.kotaKeberangkatan} → ${flight.kotaTujuan}`,
    );
    ...
    try {
      setPassengers(
        await apiGet<Passenger[]>(
          `/admin/penumpang-per-penerbangan/${flight.id}`,
        ),
      );
    } catch (caught) {
      setPassengerError(
        caught instanceof Error
          ? caught.message
          : 'Daftar penumpang belum dapat dimuat.',
      );
    } finally {
  ...
  async function exportHistory(flight: Flight) {
    ...
    try {
      await apiDownload(
        `/admin/${flight.id}/ekspor-history-update-ke-excell`,
        `histori-${flight.maskapai}-${flight.id}.xlsx`,
      );
    } catch (caught) {
      setError(
  ...
  const visibleFlights = (pageData?.content ?? []).filter((flight) => {
    ...
    return (
      !value ||
      [flight.maskapai, flight.kotaKeberangkatan, flight.kotaTujuan].some(
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="page-eyebrow">
            <PlaneTakeoff className="size-4" /> Operasional penerbangan
          </p>
          <h1 className="page-title">Data penerbangan</h1>
          <p className="page-description">
            Tambah jadwal, perbarui rute, lihat penumpang, dan ekspor histori
            perubahan.
          </p>
        </div>
        <Button
      ...
      <Card className="premium-card">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-sm">
              ...
              <Input
                ...
                placeholder="Cari maskapai atau kota…"
            ...
            <NativeSelect
              ...
              aria-label="Urutan keberangkatan"
            >
        ...
        {loading ? (
          <div className="p-5">
            <LoadingPanel label="Memuat penerbangan…" />
          </div>
        ) : error && !pageData ? (
          <div className="p-5">
          ...
          <div className="p-5">
            <EmptyPanel
              title="Penerbangan tidak ditemukan"
              description="Ubah kata pencarian atau tambahkan jadwal penerbangan baru."
            />
          </div>
        ) : (
          <>
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="pl-5">Maskapai & rute</TableHead>
                  ...
                  <TableRow key={flight.id}>
                    <TableCell className="min-w-56 pl-5">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {flight.kotaKeberangkatan} → {flight.kotaTujuan}
                      </p>
                    </TableCell>
                    <TableCell className="min-w-44">
                      <p className="font-semibold">
                        {formatDateTime(flight.waktuKeberangkatan)}
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        Tiba {formatDateTime(flight.waktuKedatangan)}
                      </p>
                    </TableCell>
                    ...
                    <TableCell>
                      <StatusBadge
                        status={
                          flight.ketersediaanPenerbangan ??
                    ...
                    <TableCell className="pr-5">
                      <div className="flex justify-end gap-1">
                        <Button
                          variant="ghost"
                          size="icon-lg"
                          onClick={() => showPassengers(flight)}
                          className="size-11 cursor-pointer"
                          aria-label={`Lihat penumpang ${flight.maskapai}`}
                        >
                          <UsersRound />
                        </Button>
                        <Button
                          variant="ghost"
                          size="icon-lg"
                          onClick={() => exportHistory(flight)}
                          disabled={exporting === flight.id}
                          className="size-11 cursor-pointer"
                          aria-label={`Ekspor histori ${flight.maskapai}`}
                        >
                          {exporting === flight.id ? (
                            <Spinner />
                          ) : (
                        ...
                        <Button
                          ...
                          className="size-11 cursor-pointer"
                          aria-label={`Edit ${flight.maskapai}`}
                        >
                          <Edit3 />
                        </Button>
                        <AlertDialog>
                          <AlertDialogTrigger
                            render={
                              <Button
                                ...
                                aria-label={`Hapus ${flight.maskapai}`}
                              />
                            }
                          >
                            <Trash2 />
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Penerbangan hanya dapat dihapus jika belum
                                memiliki booking yang terhubung.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
            ...
            <div className="flex flex-col items-center justify-between gap-3 border-t p-4 sm:flex-row sm:px-5">
              <p className="text-xs text-muted-foreground">
                Halaman {(pageData?.number ?? 0) + 1} dari{' '}
                {Math.max(pageData?.totalPages ?? 1, 1)} ·{' '}
        ...
        <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl font-normal">
              {editing ? 'Edit penerbangan' : 'Tambah penerbangan'}
            </DialogTitle>
            <DialogDescription>
              {editing
                ? 'Perbarui detail jadwal. Jumlah kursi dipertahankan agar peta kursi tetap konsisten.'
                : 'Isi seluruh detail untuk menerbitkan jadwal baru.'}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input
                  ...
                  value={form.maskapai}
                  onChange={(event) =>
                    setForm({ ...form, maskapai: event.target.value })
                  }
              ...
              <div className="space-y-2">
                <Label htmlFor="from">Kota keberangkatan</Label>
                <Input
                  id="from"
                  value={form.kotaKeberangkatan}
                  onChange={(event) =>
                    setForm({ ...form, kotaKeberangkatan: event.target.value })
                  }
                  className="h-11 rounded-xl"
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, kotaTujuan: event.target.value })
                  }
                  className="h-11 rounded-xl"
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, waktuKeberangkatan: event.target.value })
                  }
                  className="h-11 rounded-xl"
                  required
                />
              ...
              <div className="space-y-2">
                <Label htmlFor="arrival">Waktu kedatangan</Label>
                <Input
                  id="arrival"
                  type="datetime-local"
                  value={form.waktuKedatangan}
                  onChange={(event) =>
                    setForm({ ...form, waktuKedatangan: event.target.value })
                  }
                  className="h-11 rounded-xl"
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, hargaTiket: Number(event.target.value) })
                  }
                  className="h-11 rounded-xl"
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={form.kursi ?? ''}
                  onChange={(event) =>
                    setForm({ ...form, kursi: Number(event.target.value) })
                  }
                ...
                <p className="text-xs text-muted-foreground">
                  {editing
                    ? 'Tidak dapat diubah setelah peta kursi dibuat.'
                    : 'Kursi akan dibuat otomatis.'}
                </p>
              </div>
            </div>
      ...
      <Dialog open={passengerOpen} onOpenChange={setPassengerOpen}>
        <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{passengerTitle}</DialogDescription>
          </DialogHeader>
          {passengerLoading ? (
            <LoadingPanel label="Memuat penumpang…" />
          ) : passengerError ? (
            <ErrorPanel message={passengerError} />
          ) : passengers.length === 0 ? (
            <EmptyPanel
              title="Belum ada penumpang"
              description="Penumpang yang sudah booking akan muncul di sini."
            />
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {passengers.map((passenger, index) => (
                  <TableRow
                    key={`${passenger.kodeBooking ?? 'passenger'}-${index}`}
                  >
                    <TableCell className="font-bold">
                      {passenger.namaPenumpang ?? passenger.nama ?? '—'}
                    </TableCell>
                    <TableCell>
                      {passenger.email ?? passenger.noHP ?? '—'}
                    </TableCell>
                    <TableCell>{passenger.kodeBooking ?? '—'}</TableCell>
                    <TableCell>
                      {passenger.nomorKursi ?? 'Belum dipilih'}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
