...
function ProfileContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : 'Profil belum dapat dimuat.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function updatePassword(event: React.FormEvent) {
    ...
    try {
      await apiPut('/user/change-password', {
        passwordLama: passwords.current,
        passwordBaru: passwords.next,
      });
  ...
  if (loading)
    return (
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        <LoadingPanel label="Memuat profil…" />
      </div>
    );
  if (error && !profile)
    return (
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
      <div className="mb-8">
        <p className="page-eyebrow">
          <UserRound className="size-4" /> Akunmu
        </p>
        <h1 className="page-title">Profil & keamanan</h1>
        <p className="page-description">
          Perbarui nama yang tampil dan jaga keamanan akunmu.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
        <Card className="premium-card lg:self-start">
          <CardContent className="p-6 text-center">
            <span className="mx-auto grid size-20 place-items-center rounded-xl border border-primary/20 bg-[linear-gradient(145deg,rgba(223,189,118,0.18),rgba(23,50,57,0.65))] text-primary">
              <UserRound className="size-9" />
            </span>
            <h2 className="font-display mt-5 text-2xl font-normal text-foreground">
              {profile.nama}
            </h2>
            <p className="mt-1 break-all text-sm text-muted-foreground">
              {profile.email}
            </p>
            <span className="mt-5 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary">
              <ShieldCheck className="size-4" /> Akun {profile.role}
            </span>
          </CardContent>
        </Card>
              ...
              <form onSubmit={updateName} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Email tidak dapat diubah dari aplikasi.
                  </p>
                </div>
                <div className="space-y-2">
          ...
          <Card className="premium-card">
            <CardHeader className="border-b p-5 sm:p-6">
              <CardTitle className="flex items-center gap-2 text-lg font-extrabold">
                <KeyRound className="size-5 text-primary" /> Ganti password
              </CardTitle>
            </CardHeader>
            <CardContent className="p-5 sm:p-6">
              <form onSubmit={updatePassword} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Minimal 5 karakter. Disarankan tetap mengikuti pola huruf
                    kapital dan angka.
                  </p>
                </div>
                <Button
